...
def create_session(
    ...
    chat_session = session_service.create_session(
        db, user.id, payload.title, payload.model
    )
...
def list_messages(
    ...
    """查询会话历史消息，按创建顺序返回。"""
    chat_session = session_service.get_session(db, session_id, user.id)
...
def rename_session(
    ...
    """重命名会话。"""
    chat_session = session_service.rename_session(
        db, session_id, payload.title, user.id
    )
    return ok({"session": _session_payload(chat_session)})
